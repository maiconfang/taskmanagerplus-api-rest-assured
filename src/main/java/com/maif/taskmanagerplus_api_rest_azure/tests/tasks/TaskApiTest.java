...
public class TaskApiTest extends BaseTest {
    ...
    private static int taskIdNew; // Static variable to store the task ID
    ...
    public void testCreateTask() {
        ...
        taskIdNew = response.jsonPath().getInt("id"); // Extracts the ID from the response
        System.out.println("Created Task ID: " + taskIdNew);
    ...
    public void testDeleteTask() {
        ...
        given()
            .spec(TestUtil.addTokenHeader(RestAssured.given())) // Uses the helper method to add the authorization header
            .when()
            .delete(RestAssured.baseURI + TASKS_PATH + "/" + taskIdNew) // Builds the complete URL using RestAssured.baseURI and TASKS_PATH
            .then()
            .statusCode(204);
    }
